...
function drawLoops() {
    loops.forEach(function(loop) {
        ...
        const startAngle = Math.PI/4; // Start angle (45 degrees)
        const endAngle = 2*Math.PI + Math.PI/4; // Full circle plus 45 degrees
        
        // Calculate center point for the loop (top-right of the state)
        const loopCenterX = loop.state.x + loop.state.radius * Math.cos(Math.PI/4);
        const loopCenterY = loop.state.y - loop.state.radius * Math.sin(Math.PI/4);
        ...
        const arrowAngle = endAngle - 0.2; // Slightly before end of arc
        ...
        const arrowX2 = loopCenterX + loopRadius * Math.cos(arrowAngle2);
        const arrowY2 = loopCenterY + loopRadius * Math.sin(arrowAngle2);
        
        drawArrow(arrowX2, arrowY2, arrowX, arrowY, 8);
        
        // Draw the loop name near the arc
        const textX = loopCenterX;
        const textY = loopCenterY - loopRadius - 8;
